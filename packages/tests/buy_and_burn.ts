...
import {PROGRAM_ID, YieldControllerClient, BuyAndBurnState} from "../client/src";
import {
  Account,
  createMint,
...
import {BuyAndBurn} from "../client/src/types/buy_and_burn";
const program = anchor.workspace
    .BuyAndBurn as Program<BuyAndBurn>;
...
describe.only("buy-and-burn", () => {
  ...
  beforeEach(async () => {
    ...
    [yieldAccount, yieldAccountBump] = PublicKey.findProgramAddressSync(
  ...
  it("It can register a new controller state", async () => {
    ...
    const state = await program.account.state.fetch(stateAddress) as BuyAndBurnState;
