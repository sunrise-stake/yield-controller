...
import {
  InitialisedClient,
  YieldRouterClient,
} from "../packages/yield-router/client";
import {
  FundSenderClient,
  FundSenderConfig,
} from "../packages/fund-sender/client";
...
export type ConfiguredClient = FundSenderClient & {
...
export const fundSenderDestinations: Record<string, FundSenderType> = {
  ecotoken: {
    type: "cnft",
    lookupTable: new PublicKey("FmV5V5C3kd9X7bXgFCeFbfBGyt46eUMy6s2kb3rZPudm"),
  },
  toucan: { type: "spl" },
  loompact: { type: "spl" },
};
...
export const getBalance = async (
  ...
  const accountInfo =
    await yieldRouterClient.provider.connection.getAccountInfo(account);
...
export const getSplBalance = async (
  ...
  const accountInfo = await getAccount(
    yieldRouterClient.provider.connection,
    account
  );
  return accountInfo?.amount;
};

export const printBalance = (balance: number | undefined): string => {
  const solVal = balance !== undefined ? balance / 10 ** 9 : 0;
...
export let fundSenderClients: Array<
  FundSenderClient & {
    config: FundSenderConfig;
  }
>;

const getFundSenderClients = async (): Promise<ConfiguredClient[]> =>
  Promise.all(
    Object.keys(fundSenderDestinations).map(async (destinationName) => {
      ...
      return FundSenderClient.fetch(stateAddress);
    })
  );

export const getFundSenderAvailableAmount = async (
  client: FundSenderClient
): Promise<number> => {
  const inputAccount = client.getInputAccount();
  const balance = await getBalance(inputAccount);
  const minimumRentExemption =
    await client.provider.connection.getMinimumBalanceForRentExemption(0);
  return Math.max((balance ?? 0) - minimumRentExemption, 0);
};

export const getFundSenderData = async () => {
  return Promise.all(
    yieldRouterClient.config.outputYieldAccounts.map(async (a, index) => {
      const fundSenderClient = fundSenderClients.find((c) =>
        c.getInputAccount().equals(a)
      );
      const balance = await getBalance(a);
      const destinationAddress = fundSenderClient?.config.destinationAccount;
      const destinationBalance = destinationAddress
        ? await getBalance(destinationAddress)
        : undefined;
      const fundSenderThreshold =
        fundSenderClient?.config.spendThreshold?.toNumber() ?? 0;
      ...
      return {
        ...
        fundSenderName:
          fundSenderClient?.config.destinationName ??
          "Not Fund Sender Destination",
...
export const showData = async () => {
  ...
  const inputBalance = await getBalance(
    yieldRouterClient.getInputYieldAccount()
  );
...
export const selectAmount = (maxLamports: number): number => {
  ...
  if (isNaN(selectedSol) || selectedSol <= 0 || selectedSol > maxSol) {
    console.log(
      chalk.red(
        `Invalid amount. Please enter a number between 0 and ${formattedMaxSol}.`
      )
    );
