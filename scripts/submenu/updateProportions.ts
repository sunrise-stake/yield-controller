...
import { getFundSenderData, yieldRouterClient } from "../util";
import readlineSync from "readline-sync";

export const submenuUpdateProportions = async () => {
  // ask for confirmation:
  const confirm = readlineSync.question(
    chalk.yellow("Update allocation proportions? (y/n): ")
  );
  if (confirm !== "y") return;
  ...
  const newAllocations = fundSenderData.map((data, index) => {
    ...
    console.log(
      chalk.cyanBright(
        `${
          index + 1
        }: ${destinationName} Current allocation: ${currentAllocation}%. `
      )
    );

    // a regex that matches a number between 0 and 100
    return readlineSync.question("New allocation:", {
      limit: /^([0-9]|[1-9][0-9]|100)$/,
    });
  });
  ...
  newAllocations.forEach((allocation, index) => {
    console.log(
      chalk.cyanBright(
        `${fundSenderData[index].fundSenderName}: ${allocation}%`
      )
    );
  });
  const confirmUpdate = readlineSync.question(
    chalk.yellow("Confirm update (y/n): ")
  );
  ...
  await yieldRouterClient.updateOutputYieldAccounts(
    outputYieldAccounts,
    spendProportions
  );
